  ...
  return (
    options ? (
      <div ref={priceRef} className='py-4 w-full flex flex-row justify-center items-center space-x-8'>
        {enableButtons && (<button className='bg-gaia p-4 text-lg rounded-full text-slate-700' onClick={handlePrev}><IoChevronBack /></button>)}
        ...
        {enableButtons && (<button className='bg-gaia p-4 text-lg rounded-full text-slate-700' onClick={handleNext}><IoChevronForward /></button>)}
      </div>
    ) : (
